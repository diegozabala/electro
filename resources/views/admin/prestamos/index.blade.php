...
<section class="section-login">
    ...
    <div class="panel-registro-table">
        ...
        <div class="form-group col-md-3">
            <label >Portatiles: {{$portatiles}}</label>

        </div>
        <div class="form-group col-md-3">
            <label >VideoBeam: {{$vb}}</label>

        </div>
        <div class="form-group col-md-3">
            <label >Apuntadores: {{$apun}}</label>
        </div>
            ...
            @foreach($prestamos as $prestamo)
                ...
                <tr class="table-info">
                    ...
                    @if($prestamo->nombre == "EXTERNOS")
                        <td bgcolor=#f0ed64>{{$prestamo->nombre_profesor ." ".$prestamo->apellido_profesor}}</td>
                        ...
                        <td bgcolor=#f0ed64><a href="{{ route('admin.prestamos.destroy',$prestamo->id) }}" class="btn btn-danger"
                            ...
                            <a href="{{route('admin.profesores.show',$prestamo->profesores_id)}}" class="btn btn-success">
                    ...
                    @elseif($prestamo->nombre != "EXTERNOS")
                        <td>{{$prestamo->nombre_profesor ." ".$prestamo->apellido_profesor}}</td>
